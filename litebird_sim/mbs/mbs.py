...
class Mbs:
    def __init__(
        self, simulation, instrument=None, detector_list=None, channel_list=None
    ):
        self.sim = simulation
        ...
        if self.det_list:
            ...
            for d in self.det_list:
                ...
                self.instrument[name] = {
                    "freq": d.bandcenter_ghz,
                    "freq_band": d.bandwidth_ghz,
                    "beam": d.fwhm_arcmin,
                    "P_sens": d.pol_sensitivity_ukarcmin,
                }
        elif self.ch_list:
            self.instrument = {}
            try:
            ...
            for c in self.ch_list:
                ...
                name = name.replace(" ", "_")
                self.instrument[name] = {
                    "freq": c.bandcenter_ghz,
                    "freq_band": c.bandwidth_ghz,
                    "beam": c.fwhm_arcmin,
                    "P_sens": c.pol_sensitivity_channel_ukarcmin,
                }
            ...
            if self.instrument == None:
                ...
                if custom_instrument:
                    if "toml" in custom_instrument:
                        ...
                        self.instrument = np.load(
                            custom_instrument, allow_picke=True
                        ).item()
                    else:
    ...
    def check_parameters(self):
        ...
        try:
            self.save = config_mbs["general"]["save"]
        except KeyError:
            self.save = True
            log.info("setting save = True")
        try:
            self.gaussian_smooth = config_mbs["general"]["gaussian_smooth"]
        except KeyError:
            self.gaussian_smooth = False
            log.info("setting gaussian_smooth = False")
        try:
            self.bandpass_int = config_mbs["general"]["bandpass_int"]
        except KeyError:
        ...
        try:
            ...
            log.info("setting bandpass_int = False")
        try:
            self.coadd = config_mbs["general"]["coadd"]
        except KeyError:
            if self.save == True:
                self.coadd = False
                ...
                self.coadd = True
        try:
            self.parallel_mc = config_mbs["general"]["parallel_mc"]
        except KeyError:
        ...
        try:
            self.make_noise = config_mbs["noise"]["make_noise"]
        except KeyError:
        ...
        try:
            self.make_cmb = config_mbs["cmb"]["make_cmb"]
        except KeyError:
            self.make_cmb = False
        ...
        if self.make_cmb:
            ...
            try:
                ...
                self.seed_cmb = None
        try:
            self.make_fg = config_mbs["fg"]["make_fg"]
        except KeyError:
            self.make_fg = False
        ...
        try:
            self.out_dir = config_mbs["output"]["output_directory"]
        except KeyError:
            self.out_dir = "./"
        log.info(f"saving files in {self.out_dir}")
        try:
            self.out_string = config_mbs["output"]["output_string"]
        except KeyError:
        ...
        for Nchnl, chnl in enumerate(channels):
            ...
            for nmc in range(rank * perrank, (rank + 1) * perrank):
                ...
                if N_split:
                    ...
                    for hm in range(N_split):
                        ...
                        lbs.write_healpix_map_to_file(
                            file_tot_path, noise_map_split, column_units=col_units
                        )
                ...
                if self.save:
                    ...
                    file_tot_path = f"{out_dir}{nmc_str}/{file_name}"
                    lbs.write_healpix_map_to_file(
                        file_tot_path, noise_map, column_units=col_units
                    )
                else:
    ...
    def generate_cmb(self):
        ...
        for nmc in range(rank * perrank, (rank + 1) * perrank):
            ...
            file_tot_path = f"{out_dir}{nmc_str}/{file_name}"
            lbs.write_healpix_map_to_file(
                file_tot_path, cmb_temp, column_units=col_units
            )
            ...
            for Nchnl, chnl in enumerate(channels):
                ...
                if smooth:
                    cmb_map_smt = hp.smoothing(
                        cmb_map, fwhm=np.radians(fwhm / 60.0), verbose=False
                    )
                else:
                    cmb_map_smt = cmb_map
                if self.save:
                    ...
                    file_tot_path = f"{out_dir}{nmc_str}/{file_name}"
                    lbs.write_healpix_map_to_file(
                        file_tot_path, cmb_map_smt, column_units=col_units
                    )
                else:
                ...
                for Nchnl, chnl in enumerate(channels):
                    ...
                    if smooth:
                        sky_extrap_smt = hp.smoothing(
                            sky_extrap, fwhm=np.radians(fwhm / 60.0), verbose=False
                        )
                    else:
                    ...
                    if self.save:
                        ...
                        lbs.write_healpix_map_to_file(
                            file_tot_path, sky_extrap_smt, column_units=col_units
                        )
                    else:
        ...
        if os.path.exists(fg_dir):
            ...
            for chnl in channels:
                ...
                for cmp in components:
                    ...
                    try:
                        fg_cmp = hp.read_map(
                            f"{fg_dir_cmp}{fg_file_name}", (0, 1, 2), verbose=False
                        )
                    except IndexError:
                        fg_cmp = hp.read_map(
                            f"{fg_dir_cmp}{fg_file_name}", verbose=False
                        )
                        fg_cmp = np.array([fg_cmp, fg_cmp * 0.0, fg_cmp * 0.0])
                    fg_tot += fg_cmp
                if os.path.exists(cmb_dir):
                    ...
                    for nmc in range(nmc_cmb):
                        ...
                        cmb = hp.read_map(
                            f"{cmb_dir}{nmc_str}/{cmb_file_name}",
                            (0, 1, 2),
                            verbose=False,
                        )
                        map_tot = fg_tot + cmb
                        ...
                        tot_file_name = (
                            f"{chnl}_coadd_signal_map_{nmc_str}_{file_str}.fits"
                        )
                        lbs.write_healpix_map_to_file(
                            f"{coadd_dir}{nmc_str}/{tot_file_name}",
                            map_tot,
                    ...
                    lbs.write_healpix_map_to_file(
                        f"{coadd_dir}/{tot_file_name}", map_tot, column_units=col_units
                    )
