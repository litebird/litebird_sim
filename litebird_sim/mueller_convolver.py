...
class MuellerConvolver:
    ...
    class AlmPM:
        ...
        def __getitem__(self, lm):
            ...
            if isinstance(ell, slice):
                if ell.step is not None or ell.start < 0 or ell.stop - 1 > self._lmax:
                    print(ell, m)
                    raise ValueError("out of bounds read access")
            else:
                if ell < 0 or ell > self._lmax:  # or abs(m) > l:
                    print(ell, m)
                    raise ValueError("out of bounds read access")
            # if we are asked for elements outside our m range, return 0
            if m < -self._mmax or m > self._mmax:
                return 0.0 + 0j
            return self._data[m + self._mmax, ell]
        ...
        def __setitem__(self, lm, val):
            ...
            if isinstance(ell, slice):
                ...
                if (
                    ...
                    print(ell, m)
                    raise ValueError("out of bounds write access")
            self._data[m + self._mmax, ell] = val
