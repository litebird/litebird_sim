...
def fill_tod(
    ...
    Args:
        ...
        nside_out (int) : sets a different nside to the output map. Only used when
              build_map_on_the_fly is set to True.

        maps (np.ndarray) : input maps (3, npix) coherent with nside provided,
    ...
    for idx_obs, cur_obs in enumerate(obs_list):
        ...
        for idet in range(cur_obs.n_detectors):
            ...
            if build_map_on_the_fly:
                pix_out = hp.ang2pix(nside_out, cur_point[:, 0], cur_point[:, 1])
            ...
            if build_map_on_the_fly:
                atd = np.zeros((npix_out, 3), dtype=np.float64)
                ata = np.zeros((npix_out, 3, 3), dtype=np.float64)
    ...
    if build_map_on_the_fly:
        return make_map(obs_list, ata, atd, comm)


def make_map(observations, ata, atd, comm):
    ...
    if all([obs.comm is None for obs in observations]) or not mpi.MPI_ENABLED:
        ...
        comm.Allreduce(mpi.MPI.IN_PLACE, atd, mpi.MPI.SUM)
        comm.Allreduce(mpi.MPI.IN_PLACE, ata, mpi.MPI.SUM)
    else:
    ...
    res = np.full_like(atd, hp.UNSEEN)
    mask = cond < COND_THRESHOLD
    res[mask] = np.linalg.solve(ata[mask], atd[mask])
    return res.T
