...
def bandpass_chebyshev(freqs, f0, f1, order=3, ripple_dB=3):
    ...
    b, a = signal.cheby1(
        order,
        ripple_dB,
        [2.0 * np.pi * f0 * 1e9, 2.0 * np.pi * f1 * 1e9],
        "bandpass",
        analog=True,
    )
...
def bandpass_profile(
    ...
    if "bandpass_file" in bandpass.keys():
        ...
        if bandpass["band_type"] == "top_hat":
            ...
            if (
                "band_alpha" not in bandpass.keys()
                and "band_beta" not in bandpass.keys()
            ):

                profile = top_hat_bandpass_with_exp_wings(
                    freqs,
                    bandpass["band_low_edge"],
                    bandpass["band_high_edge"],
                )
            ...
            if (
                "band_order" not in bandpass.keys()
                and "band_ripple_dB" not in bandpass.keys()
            ):

                profile = bandpass_chebyshev(
                    freqs,
                    bandpass["band_low_edge"],
                    bandpass["band_high_edge"],
                )
    ...
    if (
        ...
        and bandpass["band_high_freq_leak"] == True
    ):
