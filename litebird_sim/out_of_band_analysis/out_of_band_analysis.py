    ...
    def fill_tod(
        ...
        Args:
             ...
             pointings (float): pointing for each sample and detector
                 generated by func:lbs.scanning.get_pointings. Don't add HWP
        ...
        for cur_obs, cur_ptg, cur_psi in zip(obs_list, ptg_list, psi_list):
            ...
            for idet in range(cur_obs.n_detectors):
                pix = hp.ang2pix(self.nside, cur_ptg[idet, :, 0], cur_ptg[idet, :, 1])

                if self.built_map_on_the_fly:
                    tod = np.zeros_like(cur_ptg[idet, :, 0])
                else:
                ...
                if self.integrate_in_band:
                    integrate_in_band_signal_for_one_detector(
                        ...
                        z2=self.z2,
                        pixel_ind=pix,
                        polangle=0.5 * cur_psi[idet] + times * hwp_radpsec,
                        maps=self.maps,
                    )
                else:
                ...
                if self.built_map_on_the_fly:
                    if self.correct_in_solver:
                        ...
                        ca = np.cos(2 * cur_psi[idet] + 4 * times * hwp_radpsec)
                        sa = np.sin(2 * cur_psi[idet] + 4 * times * hwp_radpsec)
                    ...
                    cur_obs.psi[idet, :] = cur_psi[idet] + 2 * times * hwp_radpsec
