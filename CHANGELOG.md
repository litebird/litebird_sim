...
-   **Breaking change**: Stop supporting Python 3.10, which reaches end of life in October 2026; the minimum is now Python 3.11. Besides the end-of-life date, 3.10 was forcing a split dependency stack: `numpy` had to be pinned to `>=2.2.6,<2.3` on 3.10 against `>=2.4.1` elsewhere, and the lock carried eleven packages at two or three versions at once, including `astropy` 6.1.7 alongside 8.0.1. The stack now resolves to a single version almost everywhere, and the `exceptiongroup` backport is no longer needed since `ExceptionGroup` is in the standard library from 3.11. Reference PR: [#555](https://github.com/litebird/litebird_sim/pull/555).
...
-   Execute the example notebooks in CI on every push and render them in the documentation under “Example notebooks”, so the code shipped in `notebooks/` is guaranteed to run against the current version of the framework. Also adds an `nbstripout` pre-commit hook that keeps notebook outputs, which are what the docs render, while stripping the execution counts and kernel metadata that made notebook diffs unreadable. Reference PR: [#542](https://github.com/litebird/litebird_sim/pull/542).
